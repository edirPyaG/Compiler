...
public class Visitor extends miniSysYBaseVisitor<Void> {
    ...
    public Void visitProgram(miniSysYParser.ProgramContext ctx) {
        ...
        PointerType ptrCharType = PointerType.getPointTy(i8Type); // 字符指针
        ...
        ArrayList<Type> params_char = new ArrayList<>(Collections.singletonList(i32Type));
        ...
        ArrayList<Type> params_putf = new ArrayList<>(Collections.singletonList(ptrCharType));
        ...
        scope.put("putch", builder.buildFunction("putch", new FunctionType(voidType, params_char), true));
        
        // === 格式化输出函数 ===
        scope.put("putf", builder.buildFunction("putf", new FunctionType(voidType, params_putf), true));
        
        // === 计时函数 ===
        scope.put("starttime", builder.buildFunction("starttime", new FunctionType(voidType, params_empty), true));
        scope.put("stoptime", builder.buildFunction("stoptime", new FunctionType(voidType, params_empty), true));
    ...
    public Void visitParam(miniSysYParser.ParamContext ctx) {
        if (ctx.STRING() != null) {
            ...
            // 创建全局字符串常量
            tmp_ = builder.buildGlobalStringPtr(str);
        } else {
            // 普通表达式参数
            visit(ctx.exp());
        }
